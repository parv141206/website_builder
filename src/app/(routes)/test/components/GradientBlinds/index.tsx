...
`;
    const {
      arr: colorArr,
      count: colorCount
    } = prepStops(memoizedGradientColors);
    const uniforms: {
      iResolution: {
    ...
    const onPointerMove = (e: MouseEvent) => {
      ...
      const scale = (renderer as unknown as {
        dpr?: number;
      }).dpr || 1;
    ...
    return () => {
      ...
      callIfFn(meshRef.current as unknown as {
        remove?: () => void;
      }, "remove");
      callIfFn(rendererRef.current as unknown as {
        destroy?: () => void;
      }, "destroy");
  ...
  }, [dpr, paused, memoizedGradientColors, angle, noise, blindCount, blindMinWidth, mouseDampening, mirrorGradient, spotlightRadius, spotlightSoftness, spotlightOpacity, distortAmount, shineDirection, height, width]);
  return <div ref={el => {
    containerRef.current = el;
  }} className={`absolute top-0 left-0 overflow-hidden ${className}`.trim()} style={{
    height,
    width,
    ...(mixBlendMode && {
